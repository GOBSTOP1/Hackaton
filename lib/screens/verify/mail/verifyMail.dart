    ...
    ? const HomeBar()
     ...
     : Scaffold(
      ...
      child: Column(
        ...
        children: [
          Text('Письмо с подтверждением отправлено на почту: $email',style: theme.textTheme.bodyMedium,),
          const SizedBox(height: 20,),
          ElevatedButton.icon(onPressed: (){canResendEmail ? sendVerificationEmail(): null;}, icon: Icon(Icons.mail), label: Text('Повторно отправить письмо',style: theme.textTheme.bodySmall,),),
          const SizedBox(height: 20,),
          TextButton(onPressed: () async{
            timer?.cancel();
            await FirebaseAuth.instance.currentUser!.delete();
          }, child: Text('Отменить',style: theme.textTheme.bodySmall,)),
        ],
      ),
      ),
    );
